...
void ObjectMgr::LoadJumpChargeParams()
{
    ...
    //                                               0   1      2                            3            4
    QueryResult result = WorldDatabase.Query("SELECT id, speed, treatSpeedAsMoveTimeSeconds, jumpGravity, comments FROM spell_jump_charge_params");
    ...
    do
    {
        ...
        if (speed <= 0.0f)
        {
            LOG_ERROR("sql.sql", "Table `spell_jump_charge_params` uses invalid speed {} for id {}, set to default charge speed {}.", speed, id, SPEED_CHARGE);
            speed = SPEED_CHARGE;
        }

        if (jumpGravity <= 0.0f)
        {
            LOG_ERROR("sql.sql", "Table `spell_jump_charge_params` uses invalid jump gravity {} for id {}, set to default {}.", jumpGravity, id, Movement::gravity);
    ...
    LOG_INFO("server.loading", ">> Loaded {} jump charge params from spell_jump_charge_params in {} ms", _jumpChargeParams.size(), GetMSTimeDiffToNow(oldMSTime));
}
