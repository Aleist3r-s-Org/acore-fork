        ...
        case INVTYPE_RELIC:
        {
            switch (proto->SubClass)
            {
                ...
                case ITEM_SUBCLASS_ARMOR_IDOL:
                    if (IsClass(CLASS_DRUID, CLASS_CONTEXT_EQUIP_RELIC) || IsClass(CLASS_MONK, CLASS_CONTEXT_EQUIP_RELIC))
                        slots[0] = EQUIPMENT_SLOT_RANGED;
                    break;
                ...
                case ITEM_SUBCLASS_ARMOR_SIGIL:
                    if (IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_EQUIP_RELIC) || IsClass(CLASS_DEMON_HUNTER, CLASS_CONTEXT_EQUIP_RELIC))
                        slots[0] = EQUIPMENT_SLOT_RANGED;
                    break;
            }
            break;
        ...
        if (pProto)
        {
            ...
            if (pItem->GetSkill() != 0)
            {
                ...
                if (pProto->Quality == ITEM_QUALITY_HEIRLOOM && pProto->Class == ITEM_CLASS_ARMOR && !HasSkill(itemSkill))
                {
                    ...
                    if (IsClass(CLASS_PALADIN, CLASS_CONTEXT_EQUIP_ARMOR_CLASS) || IsClass(CLASS_WARRIOR, CLASS_CONTEXT_EQUIP_ARMOR_CLASS) || IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_EQUIP_ARMOR_CLASS))
                    {
...
InventoryResult Player::CanRollForItemInLFG(ItemTemplate const* proto, WorldObject const* lootedObject) const
{
    ...
    if (proto->Class == ITEM_CLASS_ARMOR)
    {
        ...
        // Check for idols.
        if (proto->SubClass == ITEM_SUBCLASS_ARMOR_IDOL && !(IsClass(CLASS_DRUID, CLASS_CONTEXT_EQUIP_RELIC) || IsClass(CLASS_MONK, CLASS_CONTEXT_EQUIP_RELIC)))
        {
            return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
        }
        ...
        // Check for sigils.
        if (proto->SubClass == ITEM_SUBCLASS_ARMOR_SIGIL && !(IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_EQUIP_RELIC) || IsClass(CLASS_DEMON_HUNTER, CLASS_CONTEXT_EQUIP_RELIC)))
        {
            return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
        }
    }
        ...
        proto->InventoryType != INVTYPE_CLOAK)
    {
        ...
        else if (IsClass(CLASS_ROGUE, CLASS_CONTEXT_EQUIP_ARMOR_CLASS) || IsClass(CLASS_DEMON_HUNTER, CLASS_CONTEXT_EQUIP_ARMOR_CLASS))
        {
            if (proto->HasStat(ITEM_MOD_SPELL_POWER) || proto->HasSpellPowerStat())
            {
                return EQUIP_ERR_CANT_DO_RIGHT_NOW;
...
bool Player::LoadFromDB(ObjectGuid playerGuid, CharacterDatabaseQueryHolder const& holder)
{
    ...
    if (auto result = holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_TRANSMOG))
    {
        do
        {
            ...
            if (type < 0 || type > 19)
            {
                LOG_ERROR("custom.transmog", "Account {} has transmog with unknown type {} in account_transmog, ignoring", GetSession()->GetAccountId(), type);
                continue;
            }
...
void Player::SaveToDB(CharacterDatabaseTransaction trans, bool create, bool logout)
{
    ...
    // sAnticheatMgr->SavePlayerData(this); (TODO: check why this is commented out :P)
