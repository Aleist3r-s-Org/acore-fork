...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_DEL_TRANSMOG_SETS, "DELETE FROM `character_transmog_sets` WHERE owner = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_TRANSMOG_SETS, "SELECT `presetid`, `setname`, `setdata` FROM `character_transmog_sets` WHERE owner = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_TRANSMOG_SETS, "REPLACE INTO `character_transmog_sets` (`owner`, `presetid`, `setname`, `setdata`) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_AUCTION_ITEMS, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, itemguid, itemEntry, 0, 0, 0, transmog, enchant FROM auctionhouse ah JOIN item_instance ii ON ah.itemguid = ii.guid", CONNECTION_SYNCH);
